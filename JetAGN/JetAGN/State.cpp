...
void State::initializeParticle(Particle& p)
{
	...
	// add dimension for R
	p.ps.add(new Dimension(parameters.nR + 1, &Parameters::R, bind(initializeRPoints, _1, parameters.rmin, parameters.rmax)));
	p.ps.add(new Dimension(parameters.nR + 1, &Parameters::T, bind(initializeCrossingTimePoints, _1, parameters.rmin, parameters.rmax)));
