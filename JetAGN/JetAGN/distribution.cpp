...
/*******************bloque 2*************************/
		...
		p.ps.iterate([&p, &st, &z_position, &Eeff, &Naux](const SpaceIterator& i){
			...
			{
				//dist = N1(Eeff, t[i - 1]);  
				dist = p.distribution.interpolate({ Eeff, p.ps[1][z_position], i.its[2].peek(-1) });
				//interpola en la dimension 0, y evalua en Eeff y en un t-1 
			}
			else{
				dist = 0.0; //ver que pasa aca, sería en t[0], creo que esta bien que sea 0, no cambi ael N
			}
...
/*******************bloque 3*************************/
	...
	p.ps.iterate([&p, &st](const SpaceIterator& j){  //VER el doble iterate
		...
		p.ps.iterate([&p, &st, &delta_t, &E_position, &t_position](const SpaceIterator& i){
			...
			if (i.its[1].canPeek(-1))
			{
				...
				if (i.its[1].canPeek(-2)){
					...
					dist1 = p.distribution.interpolate({ p.ps[0][E_position], i.its[1].peek(-1), p.ps[2][t_position] })
						*(delta_t*cLight / delta_xk_1);  //Ver

					dist2 = p.distribution.get(i)*(delta_t*cLight / delta_xk); //VER si me alcanza y no necesito aux
